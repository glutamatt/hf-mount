...
impl super::VirtualFs {
        ...
        loop {
            ...
            for (prefix, result) in prefixes.iter().zip(results) {
                match result {
                    ...
                    Err(e) => warn!("Remote poll failed for prefix '{prefix}': {e}"),
                }
            }
    ...
    pub(super) fn apply_poll_diff(
        ...
        let dirs_to_invalidate_kernel: Vec<u64>;
        {
            ...
            // Only invalidate directories whose children have been loaded (children_loaded).
            // Unloaded directories contain files that appear "missing" from the inode table
            // simply because the user hasn't navigated there yet, not because they're new.
